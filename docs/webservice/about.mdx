...
Or to build the webservice, you may run the following
from the root of the project path:

```shell
$ docker build --no-cache -f ./webservice/WebService/Dockerfile .
```

The service will be listening on port `3030`. You can access
...
For instance, try the following endpoint to get a list
of the defined repositories:
